...
def __init__():
    for line in open('./user.dict','r').readlines():
        AddUserWord(line.strip('\n'))
    for line in open('./stopword.dict','r').readlines():
...
def seg(paragraph):
    ...
    for a in atoms:
        ...
        if STOPWORDS.count(i)>0: continue
        r += '%s/%s\t' % (i,a.sPOS)
    return r

def segWithTag(paragraph):
    atoms = segment(paragraph)
    r = '';
    for a in atoms:
        if len(a.sPOS) < 1: continue
...
if __name__ == "__main__":
   ...
   # print seg(p)
    print seg('我好喜欢小米，我真的好想要的送给女朋友这个')
    print segWithTag('我好喜欢小米，我真的好想要的送给女朋友这个')
